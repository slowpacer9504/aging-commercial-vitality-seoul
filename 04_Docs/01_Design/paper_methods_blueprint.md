...
1. **연도 패널 구축**
   - 서울시 상권 raw를 연도 base로 정리하고, 보조 공공데이터와 서울생활인구 외부 유입 인구를 `adm_cd-year` contract로 정리한 뒤 `panel_main_pre_vitality`까지 결합한다.
2. **활력지수 구성**
   - 활력의 네 하위 차원과 보조 종합지수를 계산하고 최종 shared panel인 `panel_main`을 발행한다.
3. **공간 구조 확인**
   - 공간가중행렬을 만들고 ESDA로 분포, 전역 자기상관, 국지 cluster, hotspot 패턴을 확인한다.
4. **비공간 기준선 추정**
   - annual TWFE로 baseline을 추정하고 residual Moran으로 공간모형 필요성을 점검한다.
5. **공간모형 추정**
...
연구의 출발점은 서울시 행정동 **연도 패널** 을 만드는 일이다. 이 단계에서는 서울시 상권분석서비스 분기 source를 연도 단위로 재집계하고, 행정안전부 주민등록인구현황·연도·정적 보조 공공데이터를 `adm_cd-year` 기준으로 정리한다.

집필 시 핵심 문장은 아래 구조면 충분하다.

> 서울시 상권분석서비스 raw를 연도 패널로 재구성하고, 행정안전부 주민등록인구현황과 보조 공공데이터를 동일한 행정동-연도 단위로 결합하여 분석용 shared panel을 구축하였다.

이때 방법론적으로 중요한 점은 다음 두 가지다.

1. 분기 자료는 그대로 회귀에 투입한 것이 아니라 연도 단위로 annualize했다.
2. 연도·정적 자료는 분기 확장 없이 직접 결합했다.
...
각 구성요소와 하위지수는 연도별 상대순위만 남기는 방식이 아니라 전체 `2019~2025 adm_cd-year` 패널 표본을 기준으로 pooled z-score 표준화한다. 따라서 구성요소 간 단위 차이는 제거하면서도 연도 간 서울 전체 활력 수준 변화는 지수에 남긴다.

### 3.3 ESDA 단계

ESDA는 “공간을 고려할 이유가 있는가”를 보여주는 탐색 단계다. 최신 연도 cross-section의 분포 지도, Global Moran's I, Bivariate Moran's I, LISA, EHSA를 통해 고령화와 활력의 공간 패턴을 제시한다.
...
TWFE는 비공간 기준선이다. 본문에서는 “연도 고정효과와 지역 고정효과를 통제한 baseline”으로 소개하고, 잔차 Moran 결과를 통해 공간모형 도입의 필요성을 연결하면 된다.

핵심 서술 포인트는 아래 세 가지다.

1. FE 구조는 `adm_cd + year`다.
2. 메인 노출변수는 `age60_resident_share`이다.
3. 잔차 Moran's I가 공간의존 잔존 여부를 보여준다.

### 3.5 SPDM 단계

SPDM은 본 연구의 main global model이다. active specification은 `W y`, `X`, `W X`를 모두 포함하는 true SDM이며, `W age60_resident_share`와 `W controls`는 annual panel에서 직접 생성한다. 계수표 자체보다 direct / indirect / total effects 표를 중심으로 해석해야 한다.
...
연도 단위 개편 이후 GTWR의 집필상 위치는 다음처럼 정리하는 것이 맞다.
...
| `02_build_seoul_year_base.R` | 서울시 상권 원자료를 연도 base로 정리 | `seoul_year_base.parquet`, `seoul_raw_review.parquet` | Methods: 데이터와 분석단위 |
| `03_build_auxiliary_covariates.R` | 보조 공공데이터를 annual auxiliary contract로 정리 | `aux_covariates.parquet`, pre-aggregation files | Methods: 통제변수 구축 |
| `04_build_golmok_survival_rate.R` | 서울시 상권분석서비스 신생기업 생존율 JSON을 annual contract로 정리 | `golmok_survival_rate.parquet`, `golmok_survival_rate_qc.csv` | Methods: 안정성 구성 |
| `05_build_registered_resident_population.R` | 행정안전부 주민등록인구현황에서 상주인구와 고령 상주비중을 annual contract로 정리 | `registered_resident_population.parquet`, mapping/QC logs | Methods: 핵심 독립변수와 인구 통제변수 |
...
| Script | 연구 역할 | 주요 산출물 | 논문 연결 |
| --- | --- | --- | --- |
| `01_build_living_population_inflow.R` | 서울생활인구 월별 ZIP에서 외부 유입 인구를 annual contract로 정리 | `living_population_external_inflow.parquet`, inflow manifest/QC | Methods: 사회적 활력 구성 |
| `01_run_gtwr_main.R` | resident-only annual local sidecar | `gtwr_main_models_<control_set>.csv`, `gtwr_local_coefficients_<control_set>.csv`, `gtwr_local_beta_panel_<control_set>.csv` | Results 후반 또는 Appendix의 local heterogeneity evidence |
...
1. 본문에서는 연도 단위 전환의 이유를 분명히 써야 한다.
2. 분기 자료는 원천 정보를 제공했지만, 분석단위는 연도 패널이라는 점을 반복해서 확인해야 한다.
