...
고령화는 근린 생활권의 필수 소비재 및 근거리 통행 의존을 높임으로써 상권의 소비 패턴과 이용 방식을 재조정하는 '구조적 재편 요인'이다. 기존 연구는 주거환경 중심의 논의(AIP 등)에 편중되거나 단편적인 매출·유동인구 분석에 머물렀으나, 본 연구는 고령화가 상업활력의 하위 차원(경제·사회·시간·구조)에 미치는 직접효과, 공간파급효과(Spillover), 그리고 국지적 시공간 이질성을 통합 규명하고자 한다.

본 프로젝트의 active canonical design은 미시적 시공간 변동을 촘촘하게 추정하기 위해 **연도 패널에서 분기 패널로 전환**되었다. 전환 목적은 상권 매출·점포·유동인구의 분기 변동을 보존하고, 연도·정적 자료는 quarter-end as-of 규칙으로 붙이는 것이다. 따라서 본 연구의 active 분석 단위는 `adm_cd x yq`이며, `year`, `quarter`, `yq`, `quarter_index`를 공통 시간 키로 유지한다.
