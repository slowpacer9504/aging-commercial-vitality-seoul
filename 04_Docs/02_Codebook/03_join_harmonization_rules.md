...
## 3) 서울 상권 raw annualization

- 서울 상권 raw 중 분기 source는 `adm_cd-year` 기준으로 annualize한다.
- additive flow는 연합계, level/share는 연평균 또는 가중 연평균을 사용한다.
- temporal/stability 구성요소는 분기 분포를 이용해 연도 단위로 재계산한다. 시간적 활력의 분기 안정성은 4개 분기 값의 변동계수 역방향 지표로 계산한다.
- Q4 업데이트형 구조 source(`소득소비`, `아파트`, `직장인구`, `집객시설`, `상권변화지표`)는 strict Q4 snapshot으로 발행한다.
- Q4 업데이트형 source에서 Q4 관측값이 없으면 같은 연도 최신분기 값으로 대체하지 않고 결측으로 둔다.
- 연도 source는 `adm_cd-year` 기준으로 직접 표준화한다.
- active base publication 이후에는 `quarter`, `yq`를 남기지 않는다.
- canonical annual base는 `seoul_year_base.parquet`다.

## 4) Auxiliary covariate 정합

- `03_build_auxiliary_covariates.R`는 auxiliary public-data sources를 `adm_cd-year` 또는 static 단위로 정리한다.
- 의료, 대형점포, senior source는 record-level pre-aggregation layer를 남기되, active panel에는 annual count 또는 annual status만 반영한다.
...
- 생활인구는 시점 인구이므로 일자-시간대 row를 누적하지 않고 `adm_cd-month` 평균 시점인구를 만든 뒤, 월평균들의 연평균을 `adm_cd-year` 값으로 결합한다.
...
- 신생기업 생존율은 `2019`, `2022`, `2025` 기준연도 Q4 요청의 3개년 block을 행정동-연도 row로 재구성하고, `survival_3y`를 active 안정성 하위지수에 사용한다.
- 생존율 코호트 분모가 0인 행정동-연도는 임의 대체하지 않고 결측으로 유지하며 `golmok_survival_rate_qc.csv`에 기록한다.
...
- 주민등록인구 월별 stock은 연평균으로, 고령비중은 월별 분모가중 연평균 비중으로 결합한다.
...
- `06_build_analysis_panel.R`는 `seoul_year_base`, `aux_covariates`, `living_population_external_inflow`, `golmok_survival_rate`, `registered_resident_population`을 `adm_cd`, `year` 기준으로 결합한다.
...
- shared annual transform은 `adm_cd` 그룹 안에서 연도순 정렬 후 계산한다.
...
- TWFE의 시간 FE는 `year`, SPDM의 panel time index도 `year`다.
- GTWR는 annual resident-only local sidecar다.

## 8) 조인 실패 정책

- `FAIL`
  - 필수 키/입력 누락
  - annualization rule 위반
  - 음수 구조 카운트 검출
  - 핵심 산출물 생성 불가
- `WARN`
  - coverage 저하
