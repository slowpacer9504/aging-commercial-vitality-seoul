...

{
  ...
  if (!file.exists(summary_path) || !file.exists(local_path)) {
    stop(
      sprintf(
        "[ERROR] Baseline GTWR outputs are required before lamda sensitivity. Run 02_Code/80_optional/gtwr/01_run_gtwr_main.R first. Missing: %s",
  ...
  if (nrow(summary_tbl) == 0L || nrow(local_tbl) == 0L) {
    stop("[ERROR] Baseline GTWR outputs are empty; run 01_run_gtwr_main.R successfully before lamda sensitivity.", call. = FALSE)
  }
