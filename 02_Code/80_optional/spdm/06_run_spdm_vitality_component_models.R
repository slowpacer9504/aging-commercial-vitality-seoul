#==============================================================================
# Script    : 06_run_spdm_vitality_component_models.R
...
# Outputs   : spdm_vitality_component_models.csv,
...
append_log(cfg$logs$model_run, sprintf("\n## [%s] 06_run_spdm_vitality_component_models", timestamp()))

if (!file.exists(cfg$paths$panel_main) || !file.exists(cfg$paths$w_queen)) {
  stop("[ERROR] Required inputs for SPDM vitality component sidecar missing", call. = FALSE)
}

component_outcomes <- value_or(cfg$vitality_component_appendix_outcomes, character())
panel <- read_panel_main_view("spdm", extra_cols = component_outcomes)
...
assert_spdm_main_controls_current(
  control_screen,
  context = "06_run_spdm_vitality_component_models",
  control_col = "control",
...
finalize_component_outputs <- function(out_coef, out_imp, out_ctrl, out_diag) {
  list(
    coefs = out_coef |>
...
build_component_fail_result <- function(spec_id,
                                        outcome,
                                        exposure,
                                        message,
                                        requested_controls,
...
run_component_spec <- function(spec_id,
                               outcome,
                               exposure,
                               panel,
                               lw,
  ...
  if (!identical(prep$status, "success") || is.null(prep$mod)) {
    return(build_component_fail_result(
      spec_id = spec_id,
      outcome = outcome,
      exposure = exposure,
...
#==============================================================================
# 2. Run Component Queen SPDM and Save Outputs
#==============================================================================

if (length(outcomes) == 0L || length(exposures) == 0L) {
  finalized <- finalize_component_outputs(
    spdm_empty_coef_tbl(),
    spdm_empty_impacts_tbl(),
    spdm_empty_controls_tbl(),
    spdm_empty_diag_tbl()
  )
  write_csv_safe(finalized$coefs, cfg$paths$spdm_vitality_component_models)
  write_csv_safe(finalized$impacts, cfg$paths$spdm_vitality_component_impacts)
  write_csv_safe(finalized$controls, cfg$paths$spdm_vitality_component_controls_used)
  write_csv_safe(finalized$diagnostics, cfg$paths$spdm_vitality_component_diagnostics)
  append_log(cfg$logs$model_run, "- Skipped: missing SPDM vitality component outcomes/exposures")
} else {
  ...
  if (is.null(w_ids)) {
    ...
    res <- run_spdm_optional_spec_jobs(
      ...
      label = "SPDM vitality component specs"
    )
    ...
    append_log(
      cfg$logs$model_run,
      sprintf("- SPDM vitality component specs attempted: %d (success=%d, failed=%d)", nrow(spec_grid), n_success, n_fail)
    )
  }
}
